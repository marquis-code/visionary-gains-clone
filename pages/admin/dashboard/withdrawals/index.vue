        ...
        <b-card class="mt-3">
          ...
          >
            ...
            <template #cell(transactionStatus)="data">
              <div class="font-medium py-4 text-sm cursor-pointer">
                <span
                  ...
                  :class="[
                    data?.item?.transactionStatus === 'Pending'
                      ? 'bg-yellow-500 text-white'
                      : data?.item?.transactionStatus === 'Approved'
                      ? 'text-white bg-green-500'
                      : data?.item?.transactionStatus === 'Declined'
            ...
            <template #cell(actions)="data">
              <div class="py-4">
                <div
                  v-if="data?.item?.transactionStatus === 'Pending'"
                  class="flex items-center gap-x-3"
                >
                  <button
                    class="text-white text-xs bg-green-500 w-full py-1.5 rounded-full"
                    @click="handleAction(data.item, 'approve')"
                  >
                    Approve
                  </button>
                  <button
                    class="text-white text-xs bg-red-500 w-full py-1.5 rounded-full"
                ...
                <div v-else>
                  <button
                    disabled
                    class="bg-black text-white py-2 text-xs px-3 rounded-full"
                  >
                    Completed
                  </button>
                </div>
      ...
      `;
        ...
        try {
          ...
          if (data?.errors) {
            ...
            this.transactionsList = data.data.getTransactions.filter(
              (itm) => itm.transactionType === "Withdrawal"
            );
            this.totalRows = this.transactionsList.length;
          }
        } finally {
