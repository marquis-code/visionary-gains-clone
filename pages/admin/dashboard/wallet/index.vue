<template>
  <main>
    <main class="space-y-10">
      ...
      >
        <div
          v-for="(item, index) in computedCards"
          :key="index"
        ...
        >
          ...
          <div
            v-if="item.value !== 'bank'"
            class="absolute top-3 right-2 cursor-pointer"
          >
            <button
              class="flex items-center gap-x-3 border-white border px-3 py-2.5 rounded-md"
        ...
        <h1 class="text-lg font-semibold text-black">
          {{
            Object.keys(selectedWallet).length
              ? `Update ${selectedWallet?.walletName} wallet`
              : "Update Wallet"
          }}
        </h1>
      </div>
      <form
        class="mt-8 gap-6 w-full space-y-6 px-6 pb-6"
    ...
    <b-modal id="update-profit-modal" hide-footer centered hide-header>
      <div class="flex justify-center items-center">
        <h1 class="text-lg font-semibold text-black">Update Profit Form</h1>
      </div>
      <form
        class="mt-8 gap-6 w-full space-y-6 px-6 pb-6"
        @submit.prevent="handleProfitUpdate"
      >
        <div class="col-span-6 sm:col-span-6">
        ...
        <div class="w-full flex justify-between items-center gap-x-4">
          ...
          <button
            type="submit"
            :disabled="!isUpdateFormEmpty || processingProfitUpdate"
            class="w-full rounded-md border text-xs disabled:cursor-not-allowed disabled:opacity-25 border-black bg-black px-12 py-3 font-medium text-white transition hover:bg-transparent focus:outline-none focus:ring"
          >
      ...
      if (process.client) {
        ...
        `;
          ...
          if (data?.errors) {
            ...
            this.$toastr.s(
              `${this.selectedWallet?.name} wallet address was successfully updated`
            );
