<template>
  <div class="bg-[#4169E1] h-screen">
    <section
      v-if="!token_sent"
      class="bg-[#4169E1] flex justify-center items-center pt-20 container mx-auto"
    >
      <div class="w-full max-w-md p-8 space-y-6 rounded-xl bg-white">
        <h1 class="text-3xl font-bold">Don't worry!</h1>
        <p class="text-gray-500 text-sm leading-loose">
          we are here to help you recover your password. Enter the email address
    ...
    <section v-else class="grid place-content-center h-screen">
      <div class="flex justify-center items-center flex-col gap-y-3">
        ...
        <nuxt-link
          to="/reset"
          class="bg-black text-white px-6 py-2 rounded-md tetx-sm"
          >Proceed to reset</nuxt-link
        >
      </div>
    </section>
  </div>
